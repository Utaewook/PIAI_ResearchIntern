...
# Anomaly Localization AUROC를 계산
...
# probability of segmentation 계산 이상 지역 검출 결과를 평가
...
# 단일 이상 정수 맵(anomaly_scorr_map)과 실제 이상 마스크에 대해 임계값에 따른 이상 지역을 계산
# 이상 지역 검출과 관련된 지표를 반환한다.
...
# 이상 탐지 여부만 평가 하고 결과를 반환한다.
def eval_det_loc_only(det_auroc_obs, epoch, gt_label_list, anomaly_score):
    gt_label = np.asarray(gt_label_list, dtype=bool)
    det_auroc, best_det_auroc = eval_det_auroc(det_auroc_obs, epoch, gt_label, anomaly_score)
