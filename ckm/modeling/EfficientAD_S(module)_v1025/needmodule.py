...
def visualize_one_sample(test_data, orig_height, orig_width, auto, stu, tea):
    with torch.no_grad():
        ...
        tea_std = torch.tensor([[[[0.7210]], [[0.5510]], [[0.1756]], [[0.3689]], [[1.1653]], [[0.5342]], [[0.5155]],
                               ...
                               device='cuda')
        q_st_start = torch.tensor(0.0331, device='cuda')
        q_st_end = torch.tensor(0.0605, device='cuda')
        q_ae_start = torch.tensor(0.000079580, device='cuda')
        q_ae_end = torch.tensor(0.0003, device='cuda')
        auto.eval()
        stu.eval()
        tea.eval()
